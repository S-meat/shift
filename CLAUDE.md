# 希望休申請アプリ 引継ぎ

## 現在の状態

- 対象月: 2026年9月～2027年3月
- 対象シート: `2026年9月`、`2026年10月`、`2026年11月`、`2026年12月`、`2027年1月`、`2027年2月`、`2027年3月`
- スプレッドシートID: `1_jq2tl3Wfx61p-J3EvfeHOQwIh1B5Qo5KP8UCCfPnVQ`
- Apps ScriptプロジェクトID: `1dxANl5b5x1W8Wk9hT_TUJ95v5Q-Zl__y6Pyb4lIYay77pDCOXYZ9X2Qa`
- Apps Script公開URL: `https://script.google.com/macros/s/AKfycbzXu3o08Hjjo1KPSPAexQ8lfOjDRh8qz3YbAgbSiw4zUOPnN1dM94saxsCrNsTdB2GA/exec`
- GitHub Pages公開URL: `https://s-meat.github.io/shift/`
- 使い方マニュアル: `https://s-meat.github.io/shift/manual.html`

## 重要な動作

1. 日付を1回タップすると希望休として選択し、送信時に「希」を記入する。
2. 日付をすばやく2回タップするとポップアップを開く。
3. ポップアップでは時間指定または有給を選べる。有給は「有」を記入する。
4. 既存値のあるシフトセルは上書きしない。
5. 送信内容と反映結果を `希望休提出ログ` に記録する。
6. 同時送信による競合を避けるため、Apps Script側でロックを取得する。
7. 再提出時は `希望休アプリ管理` の有効データと提出ログを基準に、このアプリが以前書いた値だけを消して今回の内容へ置き換える。
8. 「毎週同じ曜日」は希望休または同じ時間を一括設定でき、「期間をまとめて」は同じ時間を一括設定できる。
9. 入力途中は月・氏名別のキーでブラウザの `localStorage` に自動保存する。「入力内容を保存」を押すと `希望休入力途中保存` シートにも保存し、同じ月と氏名を選んだ際に復元する。送信成功後は両方の保存を提出済みにする。
10. 画面右上で申請月を選び、送信先シートを切り替える。
11. ハンバーガーメニューの氏名管理は暗証番号で保護し、追加・削除・並び替え・リネームを全対象月へ同期する。
12. 氏名編集画面では各行の変更前氏名を `row.dataset.from` に保持し、保存時に `{from, to}` のペアをサーバーへ渡す。既存シフト値と管理シートの氏名・セル番地は `from` を基準に追従させ、編集後の氏名文字列だけで旧データを対応づけない。
13. 氏名同期時に退避・復元する列は固定列にしない。月タブごとに `getShiftAppDates_` で当月の日付列を取得する。2026年9月は当月1日がI列、30日がAL列にあり、旧実装の固定 `E:AI` では月末データが別人の行へ残ったため。
14. `SHIFT_APP.memberRows` は実際の氏名行32行だけを指定する。小計・計画・合計に使う14、19〜23、29、38〜39、46、50〜51行を含めない。氏名を33名以上へ増やす場合は、先に月シート自体へ氏名行を追加して数式範囲も直す構造変更が必要。
15. 月シートの氏名はC列だけに書く。D列以降は日付列なので氏名を複製しない。氏名の並び替えでは、日付見出し行にある前月末・当月・翌月頭の全日付列を本人と一緒に退避・復元する。

## UI方針

- 参考: https://s-meat.github.io/report/
- 白背景、黒文字、細い罫線、番号付きセクション。
- スマートフォン優先。操作説明は短くする。
- SAIBOKUロゴは `assets/saiboku-logo.png` を再圧縮せず使用する。
- マニュアルは `manual.html`。受付期間、操作、QRコード、Googleアカウント要否を変更した場合は同時に更新する。
- 印刷配布用PDFは `output/pdf/shift-app-employee-guide.pdf`。

## 変更時の注意

- Apps Script側の `gas/index.html` を変更したら、新バージョンとして再デプロイする。
- Apps ScriptのデプロイIDは維持し、公開URLを変えない。
- GitHub Pages側はApps Scriptをiframeで表示するため、`WebApp.gs` の `ALLOWALL` を削除しない。
- 送信試験を行う場合はテストセルと提出ログを確認し、運用データを残さない。
- 再提出は差分追加ではなく全置換。利用者向け説明では「残したい日もすべて選び直す」と明記する。
- 完了報告は推測で行わず、公開画面と対象シートの両方を確認する。

## 自動テスト

`node tests/app-api.test.js` で、旧日付の消去、有給の「有」反映、古い時間から新しい時間への置換、氏名のリネーム・削除・並び替え時のシフト値と提出履歴の追従を確認できる。9月30日相当のAL列まで含め、固定31列の外にある月末データもテストする。

## 2027年4月以降の更新

新しい月を追加する場合は、スプレッドシートへ月タブを作成し、`gas/AppApi.gs` の `SHIFT_APP.monthSheets` へタブ名を追加する。`希望休メンバー` の並び順と32名の登録上限を維持し、Apps Scriptへ反映後に新バージョンをデプロイする。
