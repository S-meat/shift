...
14. `SHIFT_APP.memberRows` は実際の氏名行だけを指定する。現在は8〜49行内の33名分が上限で、50・51行は集計行。51行以降を氏名枠に含めると集計式を上書きするため追加しない。
...
新しい月を追加する場合は、スプレッドシートへ月タブを作成し、`gas/AppApi.gs` の `SHIFT_APP.monthSheets` へタブ名を追加する。`希望休メンバー` の並び順と33名の登録上限を維持し、Apps Scriptへ反映後に新バージョンをデプロイする。
